...
export default function CampusAdminCRM() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        // Campus Admin endpoint is scoped; limit for safety
        const resp = await api.get('/api/orders', { headers, params: { limit: 1000 } });
        setOrders(Array.isArray(resp.data) ? resp.data : []);
      } catch (err) {
        const handled = handleError(err, 'CampusAdminCRM - fetchOrders');
  ...
  const topRestaurantsData = { labels: topRestaurants.labels, datasets: [{ label: 'Revenue (Rs)', data: topRestaurants.values, backgroundColor: '#A78BFA' }] };
  ...
  return (
    <div className="campus-dashboard-container">
      <div className="dashboard-header">
        <h2>📈 CRM Overview (Campus)</h2>
        <p className="text-muted">Auto-refreshing every 15 seconds • {userData?.campusName || 'Your campus'}</p>
      </div>
