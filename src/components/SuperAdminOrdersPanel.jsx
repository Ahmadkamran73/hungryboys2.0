...
export default function SuperAdminOrdersPanel() {
  ...
  const exportAggregatedOrdersByDate = () => {
    ...
    try {
      ...
      // Download file
      const filename = `aggregated_orders_all_${exportDate}.xlsx`;
      XLSX.writeFile(wb, filename);
      ...
      const handledError = handleError(e, 'SuperAdminOrdersPanel - exportAggregatedOrdersByDate');
      setError(handledError.message);
    }
  };
  ...
  const exportAggregatedOrdersToPDF = () => {
    ...
    try {
      ...
      doc.text('Aggregated Orders Report (All Campuses)', 14, 20);
      
      // Add date info
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      doc.text(`Date: ${new Date(exportDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`, 14, 30);
      doc.text(`Total Orders: ${ordersForDate.length}`, 14, 36);
      ...
      // Download PDF
      const filename = `aggregated_orders_all_${exportDate}.pdf`;
      doc.save(filename);
      ...
      const handledError = handleError(e, 'SuperAdminOrdersPanel - exportAggregatedOrdersToPDF');
      setError(handledError.message);
    }
  };
