...
export default function OrdersPanel({ campusId, campusName }) {
  const { user } = useAuth();
  ...
  // Optionally refetch when restaurant filter changes to reduce payload from server
  useEffect(() => {
    if (!campusId) return;
    if (!user) return;
    fetchOrders();
  ...
  useEffect(() => {
    // Load restaurants for this campus to populate filter options
    const loadRestaurants = async () => {
      ...
      try {
        ...
        // silently ignore; filter will be a free-text fallback
      }
    };
    loadRestaurants();
  }, [campusId]);
  ...
  const fetchOrders = async () => {
    ...
    setError('');
    try {
      // Backend automatically filters by campus based on user profile
      ...
      const handledError = handleError(e, 'OrdersPanel - fetchOrders');
  ...
  const updateStatus = async (id, status) => {
    ...
    try {
      ...
      const handledError = handleError(e, 'OrdersPanel - updateStatus');
      setError(handledError.message);
    }
  };
    ...
    try {
      const rows = filteredOrders.map((o) => {
        ...
        return {
          ...
          Campus: o.campusName || campusName || '',
      ...
      const filename = `orders_${safeCampus}_${new Date().toISOString().slice(0,10)}.xlsx`;
      XLSX.writeFile(wb, filename);
    } catch (e) {
      const handledError = handleError(e, 'OrdersPanel - exportToExcel');
      setError(handledError.message);
    }
  };
          ...
          <div className="col-md-3">
            ...
            <select
              ...
              onChange={(e)=>{
                ...
                // Optionally refetch to reduce payload
                // fetchOrders(); // uncomment if you prefer server-side reload immediately
              }}
            >
              <option value="">All</option>
              {restaurants.map(r => (
            ...
            <div className="modal-body-custom">
              <div className="row">
                ...
                <div className="col-md-6">
                  ...
                  <p>
                    <strong>Campus:</strong> {selectedOrder.campusName || campusName}
                  </p>
                  <p>
